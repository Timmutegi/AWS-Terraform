...
data "archive_file" "zip_the_python_code" {
  ...
  output_path = "${path.module}/lambda2/hello-python.zip"
}

resource "aws_lambda_function" "lambda_function" {
  # If the file is not in the current working directory you will need to include a 
  # path.module in the filename.
  filename      = "${path.module}/lambda2/hello-python.zip"
  function_name = "AclarionFunction"
...
resource "aws_api_gateway_integration" "lambdapy" {
  ...
  request_templates = {
    ...
    {
      "hour" : $util.urlDecode($input.params('hour'))
    }
    EOF
  }
}
